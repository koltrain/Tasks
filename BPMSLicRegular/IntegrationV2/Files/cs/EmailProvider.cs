namespace IntegrationV2
{
	...
	public class EmailProvider : IEmailProvider
	{
		...
		public string Send(Email message, IEnumerable<Attachment> attachments, Credentials credentials) {
			var perfomanceSend = GetIsFeatureEnabled("PerformanceOptimizedSend");
			var version = perfomanceSend ? 2 : 1;
			string serviceUri = ExchangeListenerActions.GetSendEmailUrl(_userConnection, version);
			var command = new SendCommand() {
				Email = message,
				Credentials = credentials
			};
			if (!perfomanceSend && credentials.ServerTypeId == MailServer.ExchangeTypeId) {
				//TODO CRM-57856
				command.Attachments = message.Attachments;
				command.Email.Attachments = null;
			}
