namespace Terrasoft.Configuration
{
	...
	public class GKILicensingLDAPService : BaseService
	{
				...
				foreach (var groupItem in groupList)
				{
					...
					else
					{
						SyncLDAPResponse syncLDAPResponse = JsonConvert.DeserializeObject<SyncLDAPResponse>(
							GKILicenseLDAPHttpRequest(winInstanceUrl, GKIGetSyncLDAPResponseUrl, JsonConvert.SerializeObject(filter)));
		...
		private string GKIAuthorize(string baseUrl)
		{
			...
			string authMessage = String.Format(authTemplate, authLogin, authPassword);
		...
		private static readonly string gkiLdapGroupMacroName = "[#LDAPGroupDN#]";
		private static readonly string GKIGetSyncLDAPResponseUrl = "/0/rest/GKILicensingLDAPService/GKIGetSyncLDAPResponse";
