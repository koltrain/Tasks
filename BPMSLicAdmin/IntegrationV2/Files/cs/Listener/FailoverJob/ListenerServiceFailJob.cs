namespace Terrasoft.Configuration
{
	...
	public class ListenerServiceFailJob: IJobExecutor
	{
		...
		protected List<Mailbox> GetSynchronizableMailboxes() {
			...
			var result = GetIsFeatureEnabledForAnyUser(UserConnection, "EmailIntegrationV2")
				? mailboxService.GetAllSynchronizableMailboxes()
				: mailboxService.GetExchangeMailboxes();
		...
		/// <summary>
		/// Filter <see cref="Mailbox"/> collection with licensed owners.
		/// </summary>
		/// <param name="mailboxes"><see cref="Mailbox"/> collection.</param>
		/// <returns><see cref="Mailbox"/> collection with licensed owners.</returns>
		protected List<Mailbox> FilterMailboxesWithLicensedOwners(List<Mailbox> mailboxes) {
			_log.DebugFormat("FilterMailboxesWithUnlicensedOwners started");
			LicHelper licHelper = UserConnection.LicHelper;
			return mailboxes.Where(m => licHelper.GetUserHasOperationLicense(m.OwnerId, "Login")).ToList();
		}
		...
		public void Execute(UserConnection userConnection, IDictionary<string, object> parameters) {
			...
			try {
				...
				if (!GetIsFeatureEnabledForAnyUser(userConnection, "ExchangeListenerEnabled")) {
					return;
				}
